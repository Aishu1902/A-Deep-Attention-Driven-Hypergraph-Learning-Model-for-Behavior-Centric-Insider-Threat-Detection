...
class EarlyStopping:
    ...
    def draw_trend(self,train_list,test_list):
        ...
        '''#epsilon = 1e-5  # Small value to avoid identical limits
        #plt.ylim(min(test_list) - epsilon, max(test_list) + epsilon)  
        plt.ylim(min(test_list),max(test_list)) # consistent scale
        plt.xlim(0, len(test_list)+1) # consistent scale'''
        y_min, y_max = min(test_list), max(test_list)

        if y_min == y_max:  
            y_min -= 0.01  # Add a small buffer
            y_max += 0.01

        plt.ylim(y_min, y_max)  # Set y-axis range
        plt.xlim(0, len(test_list) + 1)  # Set x-axis range
